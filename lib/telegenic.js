...
exports.makeSlideshow = function() {
  ...
  for (var i = 0; i < images.length; i++) {
    ...
    ffmpeg(images[i]).loop(5).size(resolution).aspect(aspectRatio).videoFilters(["scale=" + width + ":" + height + ":force_original_aspect_ratio=decrease", "pad=" + width + ":" + height + ":(ow-iw)/2:(oh-ih)/2"]).on('error', function(err) {
